...
import { InstalledWorkflowsResponse } from '@/types/workflow'; // Import the new interface
import { toast } from 'sonner';
import { Activity, Github, Filter, X, ChevronDown, Search, SlidersHorizontal } from 'lucide-react';
...
interface EnhancedGitHubWorkflow extends GitHubWorkflow {
  trigger_type: string;
  node_count: number;
  category: string;
...
export default function DashboardPage() {
  const [workflows, setWorkflows] = useState<EnhancedGitHubWorkflow[]>([]);
  const [loading, setLoading] = useState(true);
  const [previewWorkflow, setPreviewWorkflow] = useState<WorkflowData | null>(null);
  const [installedWorkflowNames, setInstalledWorkflowNames] = useState<Set<string>>(new Set<string>());
  const [activeN8nWorkflows, setActiveN8nWorkflows] = useState(0);
  const [githubTemplateCount, setGithubTemplateCount] = useState(0);
  ...
  const analyzeWorkflow = (workflowData: WorkflowData, fileName: string): Partial<EnhancedGitHubWorkflow> => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [githubResponse, installedResponse, n8nStatsResponse] = await Promise.all([
      ...
      const fetchedWorkflows: GitHubWorkflow[] = githubData.workflows || [];

      // Enhanced workflow processing with pre-fetched workflow_data
      const workflowsWithDetails = fetchedWorkflows.map((workflow): EnhancedGitHubWorkflow => {
        const workflowData = workflow.workflow_data;
        if (!workflowData) {
          console.warn(`Workflow data missing for ${workflow.name}. Returning fallback data.`);
          return {
            ...workflow,
          ...
          } as EnhancedGitHubWorkflow;
        }
        const analysis = analyzeWorkflow(workflowData, workflow.name);
        return {
          ...workflow,
          ...analysis,
          workflow_data: workflowData // Ensure workflow_data is carried over
        } as EnhancedGitHubWorkflow;
      });

      setWorkflows(workflowsWithDetails);
      setGithubTemplateCount(workflowsWithDetails.length);
      ...
      // Handle installed workflows
      if (!installedResponse.ok) throw new Error('Failed to fetch installed workflows');
      const installedData: InstalledWorkflowsResponse = await installedResponse.json();
      const activeInstalledNames = new Set(
        installedData.installedWorkflows
          .filter(w => w.active) // 'w' is now correctly typed as N8NWorkflow
          .map(w => formatWorkflowName(w.name))
      );
      setInstalledWorkflowNames(activeInstalledNames);

      // Handle n8n stats
      if (!n8nStatsResponse.ok) throw new Error('Failed to fetch n8n stats');
      const n8nStatsData = await n8nStatsResponse.json();
      setActiveN8nWorkflows(n8nStatsData.activeWorkflows);
  ...
  const filteredAndSortedWorkflows = useMemo(() => {
    let filtered = workflows.filter(workflow => {
      ...
      const matchesSearch = !searchTerm || 
        formatWorkflowName(workflow.name).toLowerCase().includes(searchTerm) ||
      ...
      const matchesInstalledOnly = !filters.installedOnly || 
        installedWorkflowNames.has(workflowName);

      return matchesSearch && matchesTrigger && matchesCategory && 
             matchesComplexity && matchesTags && matchesNodeCount && 
             matchesActiveOnly && matchesInstalledOnly;
    });
    ...
    filtered.sort((a, b) => {
      ...
      switch (filters.sortBy) {
        case 'name':
          comparison = formatWorkflowName(a.name).localeCompare(formatWorkflowName(b.name));
          break;
        case 'date':
    ...
    return filtered;
  }, [workflows, filters, installedWorkflowNames]);
  ...
  const handleInstall = async (workflow: EnhancedGitHubWorkflow) => {
    try {
      ...
      toast.success(`Workflow "${formatWorkflowName(workflow.name)}" installed successfully!`);
      // Refresh installed workflows
      setInstalledWorkflowNames(prev => new Set([...prev, formatWorkflowName(workflow.name)]));
    } catch (error) {
  ...
  const handleDownload = async (workflow: EnhancedGitHubWorkflow) => {
    try {
      ...
      toast.success(`Workflow "${formatWorkflowName(workflow.name)}" downloaded successfully!`);
    } catch (error) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-3 mb-8">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Currently running on n8n</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">GitHub Templates</CardTitle>
            <Github className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{githubTemplateCount}</div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => updateFilter('installedOnly', !!checked)}
                          />
                          <Label htmlFor="installedOnly" className="text-sm cursor-pointer">
                            Installed only
                          </Label>
                        </div>
                      </div>
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            ...
            {filters.installedOnly && (
              <Badge variant="secondary" className="flex items-center gap-1">
                Installed Only
                <X className="h-3 w-3 cursor-pointer hover:bg-gray-200 rounded" onClick={() => updateFilter('installedOnly', false)} />
              </Badge>
            )}
      ...
      {filteredAndSortedWorkflows.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400 mb-4">
            {workflows.length === 0 
              ? 'Try checking your GitHub repository configuration'
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => updateFilter('installedOnly', true)}
                className="text-xs"
              >
                My Workflows
              </Button>
      ...
      <div className="mt-12 pt-8 border-t border-gray-200">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          ...
          <div>
            <div className="text-2xl font-bold text-gray-900">{installedWorkflowNames.size}</div>
